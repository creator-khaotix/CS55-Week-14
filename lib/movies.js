...
export async function getMovieData(idRequested) {
  ...
  console.log(`[DEBUG] Movie: ${objReturned.slug || idRequested}`);
  console.log('[DEBUG] Has _embedded?', !!objReturned._embedded);
  console.log('[DEBUG] Has featuredmedia?', !!objReturned._embedded?.['wp:featuredmedia']);
  ...
  if (objReturned._embedded && objReturned._embedded['wp:featuredmedia']) {
    ...
    console.log('[DEBUG] No featured image found for this post');
  }
